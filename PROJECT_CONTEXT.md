...
## 홈 로그인 후 AI 맞춤 추천 확장 (2026-09-11)

- 사용자 지정 Figma 개발 > 홈/로그인 후 > Section `1473:8571` 기준. HomePage를 복제하지 않고 authenticated 상태에만 기존 AiRecommendSection을 렌더한다. 로그인 폼·다른 홈 섹션은 추가/변경하지 않았다.
- 402px 기준 섹션 748px, 카드 354×605px, 원본 판멍이 79×67px, 안내 배너 313×39px, 추천 행 304px/텍스트 276px을 적용. 원본 제목·본문·카테고리·일치/불일치 태그와 명시적 줄바꿈을 반영했다. 좁은 모바일에서는 높이가 늘어나며 내용이 잘리지 않는다.
- 예시 데이터는 homeContent.ts에 분리. 9건은 시안용 총수이며 목록은 3건 미리보기, 두 계정은 같은 예시를 사용하고 이름만 현재 데모 계정에 맞춘다. 실제 추천 API는 없음. 상단 소개는 기존 /chatbot 연결 유지, 추천 사건별 상세와 더보기 목적지는 미정이므로 임의 링크를 만들지 않고 더보기는 비활성 표시.
- 격리 Edge 360/390/402/430/1440px에서 로그인 전 미노출·로그인 후 3개 목록/에셋 표시, 가로 넘침 없음, 챗봇 이동 확인. 변경 TSX/데이터 ESLint 통과. 전체 빌드/린트는 기존 AfterStoryPage.tsx:88의 미사용 navigate 오류로 실패했으며 이번 홈 작업에서 수정하지 않았다.

## 후일담 내용 작성하기 색상 보정 (2026-09-11)

- 사용자 요청으로 `afterstory-field__demo`의 밑줄을 제거하고 기본·hover 글자색을 기존 브랜드 `--orange`(#FF9524)로 통일했다. 14px 크기·굵기·포커스 표시·자동 입력 동작은 유지했다.
- 격리 Edge에서 실제 CSS를 적용해 기본/hover 색상 rgb(255,149,36), text-decoration none, 14px 유지 확인. git diff --check 통과. CSS만 변경했으며 전체 앱 빌드·실기기 검증은 이번 범위에서 실행하지 않았다.
