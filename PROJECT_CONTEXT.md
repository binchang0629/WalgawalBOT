...
- 버튼을 58 → 56px로, 머리를 48 → 56px로 키워 시안과 맞췄다. `border-radius: 50%`는 배경이 없어도 focus 링을 둥글게 유지하려고 남겼다. 위치(우측 16px, 하단 내비 + 16px)와 탭 영역은 건드리지 않았다. 시안은 내비에 딱 붙은 +0이라 16px 차이가 남아 있다. → 필요하면 조정
