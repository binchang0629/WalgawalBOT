...
/**
 * 지훈 시연용 예시 사건. `더미 텍스트 입력`이 이 글을 본문 칸에 채운다.
 * Figma 개발 페이지 `지훈01 / 사건 작성 · 기본`(node 1446:9899)의 확정 본문 그대로다.
 */
export const JIHUN_CONTENT = '디자인 외주를 맡아서 총 320만 원으로 계약했고 처음에 계약금 100만 원은 받았습니다. 작업 끝나고 최종 파일까지 전부 전달했는데 남은 220만 원을 아직 못 받고 있어요. 원래 최종 파일 전달하면 잔금을 지급하기로 했는데 계속 조금만 기다려달라고 하고 정확히 언제 준다는 말은 안 해줍니다.\n\n그런데 확인해보니까 제가 만든 디자인은 이미 업체에서 실제로 사용하고 있어요. 파일 전달한 지도 꽤 됐고 몇 번 연락해서 잔금 언제 받을 수 있냐고 물어봤는데 답장이 늦거나 아예 없는 경우도 있었습니다. 계약 내용이랑 카톡 대화, 파일 전달한 기록은 가지고 있습니다.\n\n이미 결과물은 사용하고 있는데 돈을 계속 안 주는 상황이라 제가 어떻게 대응해야 하는지, 잔금을 받을 수 있는 방법이 있는지 궁금합니다.'
