...
function BalanceRound({ questionIndex, onNext }: { questionIndex: number; onNext: () => void }) {
  ...
  function handlePointerDown(event: PointerEvent<HTMLDivElement>) {
    ...
    drag.current = {
      id: event.pointerId, x: event.clientX, y: event.clientY, scale,
        ...
        */}
        ...
        {phase === 'result' && (
          <button
            type="button"
            className={
              'balance-section__refresh' +
              (RETRY_STYLE === 'circle' ? ' balance-section__refresh--circle' : '')
            }
            aria-label={gameName + ' 게임 다시 하기'}
            onClick={handleReset}
          >
            <img
              className="balance-section__refresh-arrow"
              src={homeIcons.refreshIcon1}
              alt=""
              aria-hidden="true"
            />
          </button>
